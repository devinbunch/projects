...
pacman::p_load(httr, readxl, here,tinytex,tidyverse,data.table,rlang,
               ggplot2, janitor, tidyfast, dtplyr, microbenchmark, skimr, lubridate, ggiraph,naniar,dtplyr, ggthemes, hrbrthemes)
pacman::p_load(tidyverse, data.table, janitor, hrbrthemes, countrycode, plotly)
...
#now plot it 
p = coal_country %>%
  ggplot(aes(x=yr_qtr, y=total, fill= country, col = country)) + 
  scale_y_continuous(labels = scales::comma) +
  geom_area() +
...
coal_country =
  coal_country %>%
   ## Use ISO abbreviations
  mutate(iso = ifelse(country!="Other", 
                          countrycode(country, 'country.name', 'iso3c'), 
                          "Other"))
## Create a copy of our faceting variable (here: iso)
coal_country$iso_copy = coal_country$iso
...
p2 =
  ...
  labs(y = "Short tons (millions)", title = "US coal exports") +
